...
constexpr unsigned short TOOJPEG_BLOCKSIZE = 256;
...
namespace TooJpeg
{
  ...
  // overload func
  bool writeJpeg(
      TooJPEG_Controller* output, const void* pixels, unsigned short width, unsigned short height,
      bool isRGB = true, unsigned char quality = 90, bool downsample = false, const char* comment = nullptr
  );
} // namespace TooJpeg
...
private:
    bool isReady = false;
    std::vector<unsigned char>* memoryStore = nullptr;
    struct TooJPEG_MemoryBlock* memoryPtr = nullptr;
    virtual void CB_Func(unsigned char b);
};
